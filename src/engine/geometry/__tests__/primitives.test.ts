...
describe('createHexagonHolePath', () => {
  it('returns a path with 6 sides plus close', () => {
    const path = createHexagonHolePath(5, 0, 0)
    const points = path.getPoints(1)
    // Hexagon: 6 sides + close = 7 points
    expect(points.length).toBe(7)
  })

  it('stays within bounding radius', () => {
    const radius = 8
    const cx = 10
    const cz = 15
...
describe('mergeGeometries', () => {
  ...
  it('merges three geometries with correct vertex and index offsets', () => {
    ...
    expect(merged.attributes.position.count).toBe(9) // 3 * 3 vertices
    expect(merged.index?.count).toBe(9) // 3 * 3 indices

    // Verify index offsets: 3rd geometry's first index should reference vertex 6
  ...
  it('merges mix of indexed and non-indexed geometries', () => {
    ...
    // Non-indexed geometry's sequential indices should be offset by 3
    const idx = merged.index!.array // eslint-disable-line @typescript-eslint/no-non-null-assertion
    expect(idx[3]).toBe(3)
    expect(idx[4]).toBe(4)
    expect(idx[5]).toBe(5)
